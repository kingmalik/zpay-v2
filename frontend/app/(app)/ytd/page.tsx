...
export default function YTDPage() {
  ...
  useEffect(() => {
    api.get<YTDData>('/api/data/ytd')
      .then(setData)
      .catch(e => setFetchError(e instanceof Error ? e.message : 'Failed to load YTD data'))
    ...
    return (
      <div className="max-w-7xl mx-auto py-10">
        <div className="rounded-2xl p-6 bg-red-500/10 border border-red-500/20 text-center">
          <p className="text-red-400 font-medium">Failed to load YTD data</p>
          <p className="text-sm dark:text-white/40 text-gray-400 mt-1">{fetchError}</p>
          <button
            onClick={() => { setFetchError(null); setLoading(true); api.get<YTDData>('/api/data/ytd').then(setData).catch(e => setFetchError(e instanceof Error ? e.message : 'Failed')).finally(() => setLoading(false)) }}
