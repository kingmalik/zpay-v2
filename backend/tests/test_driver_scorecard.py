...
def test_compute_all_drivers_with_cache_populated():
    ...
    # Five execute() calls in order: fleet, status, call_events, revenue, (cache mocked separately)
    db = MagicMock()
    db.execute.side_effect = [
        _make_execute_result([fleet_row]),   # Query 1: fleet rows
        _make_execute_result([]),            # Query 2: status events (none)
        _make_execute_result([]),            # Query 5: call events (none — pre-fix week)
...
def test_compute_all_drivers_with_empty_cache():
    ...
    db.execute.side_effect = [
        ...
        _make_execute_result([]),            # Query 5: call events (none)
...
def test_compute_all_drivers_cache_db_error_degrades_gracefully():
    ...
    db.execute.side_effect = [
        ...
        _make_execute_result([]),            # status events
        _make_execute_result([]),            # call events (none)
        _make_execute_result([]),            # revenue
    ]
...
# ── Gap-2 tests: event-driven responsiveness axis ────────────────────────────
# Tests 17-20: _compute_responsiveness with call_events

def test_responsiveness_all_answered():
    """5 attempted + 5 answered → raw = 1.0."""
    events = (
        [{"event_type": "call_attempted"}] * 5
        + [{"event_type": "call_answered"}] * 5
    )
    raw, n = _compute_responsiveness([], call_events=events)
    assert n == 5
    assert raw == pytest.approx(1.0)


def test_responsiveness_partial_answered():
    """5 attempted + 2 answered → raw = 0.4."""
    events = (
        [{"event_type": "call_attempted"}] * 5
        + [{"event_type": "call_answered"}] * 2
    )
    raw, n = _compute_responsiveness([], call_events=events)
    assert n == 5
...
def test_responsiveness_no_calls_no_penalty():
    """0 attempted, no events, no legacy timestamps → raw = 1.0, n = 0."""
    trips = [_perfect_trip(trip_ref=f"T00{i}") for i in range(5)]
    raw, n = _compute_responsiveness(trips, call_events=[])
    assert raw == pytest.approx(1.0)
    assert n == 0


def test_responsiveness_legacy_fallback():
    """No events but trips have accept_call_at set → legacy mode, raw = 1.0.

    This protects historical weeks that pre-date the Gap-2 fix.
    """
    from datetime import datetime, timezone
    call_ts = datetime(2026, 4, 21, 9, 0, tzinfo=timezone.utc)
    trips = [
        _make_trip(trip_ref=f"T00{i}", accept_call_at=call_ts)
        for i in range(5)
    ]
    # Explicitly pass empty events list to trigger legacy path
    raw, n = _compute_responsiveness(trips, call_events=[])
    assert raw == pytest.approx(1.0)
