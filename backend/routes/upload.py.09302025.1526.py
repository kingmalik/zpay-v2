...
def _parse_dt_column(df: pd.DataFrame, col: str, min_success_ratio: float = 0.6) -> pd.Series:
    ...
    # Non-blank mask
    nb_mask = s_tok.str.strip().replace({"": None}).notna()
    if not nb_mask.any():
...
def _clean_rows(rows: pd.DataFrame, fallback_text: str | None = None) -> pd.DataFrame:
    # ... your existing normalization/aliasing first
    rows.columns = _normalize_cols(list(rows.columns))
    rows = _apply_aliases(rows, CANONICAL)

    # --- NEW: if caller provided a default date, apply it to ALL rows up-front
    if fallback_text:
        # try strict parse first
        inferred = pd.to_datetime(str(fallback_text), errors="coerce", utc=True)
        if pd.isna(inferred):
            # try filename-like inference
            maybe = _infer_date_from_text(str(fallback_text))
            if maybe is not None:
                inferred = maybe
        if pd.notna(inferred):
            # set for every row (Series for alignment)
            rows["ride_start_ts"] = pd.Series([inferred] * len(rows), index=rows.index)

    _require_columns(rows, ["ride_start_ts"])

    # now run the robust parser (it’s a no-op if already set to a Timestamp)
...
def upload(file: UploadFile = File(...), default_date: Annotated[Optional[str], Form(None)] = None, -> HTMLResponse:
