...
from ..services.pdf_reader import extract_tables, normalize_details_tables
...
CONFIGS = {
    "acumen": Path("app/configs/sources/acumen.yml"),
    "acl": Path("app/configs/sources/acl.yml"),
}
...
@router.post("/acumen")
async def upload_acumen(file: UploadFile, db: Session = Depends(get_db)):
    temp_path = save_temp_file(file)
    cfg_path = CONFIGS["acumen"]
    result = crud.import_payroll_excel(db, str(temp_path), str(cfg_path))
    return {"source": "acumen", **result}

@router.post("/acl")
async def upload_acl(file: UploadFile, db: Session = Depends(get_db)):
    temp_path = save_temp_file(file)
    cfg_path = CONFIGS["acl"]
    result = crud.import_payroll_excel(db, str(temp_path), str(cfg_path))
